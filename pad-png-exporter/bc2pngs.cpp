...
void write_jpg_file(const TextureInfo &info) {
    ...
    bc_file_name << info.name << "-" << rand() << ".JPG";
    
    FILE *fp = fopen(bc_file_name.str().c_str(), "wb");
    if (!fp) {
        printf("[write_jpg_file] File %s cound not be opened for writing", info.name);
        return;
    }
    cout << "Writing " << info.name << " ..." << endl;
    if (raw_data_length != fwrite(raw_data, 1, raw_data_length, fp)) {
        printf("[write_jpg_file] Failed to write file %s", info.name);
        return;
    }
    fclose(fp);
}
...
void write_pvr_file(const TextureInfo &info) {
    ...
    if (!fp) {
        printf("[write_pvr_file] File %s cound not be opened for writing", file_name.c_str());
        return;
    }
    cout << "Writing " << file_name << " ..." << endl;
    if (raw_data_length != fwrite(raw_data, 1, raw_data_length, fp)) {
        printf("[write_pvr_file] Failed to write file %s", info.name);
        return;
    }
    fclose(fp);
}
...
{
    ostringstream bc_file_name;
    bc_file_name << info.name << "-" << rand() << ".PNG";
    
    /* create file */
    FILE *fp = fopen(bc_file_name.str().c_str(), "wb");
    if (!fp) {
        printf("[write_png_file] File %s could not be opened for writing", info.name);
        return;
    }
    printf("Writing %s ...\n", info.name);
    /* initialize stuff */
...
void process_tex1(const char *file_name, long file_size, FILE *fp)
{
    ...
    for (TextureInfoList::const_iterator it=TextureInfos.begin(); it!=TextureInfos.end(); ++it) {
        if (fseek(fp, it->offset, SEEK_SET)) {
            printf("[process_bc_file] Failed to read data from file: %s", file_name);
            return;
        }
        if (raw_data) {
            free(raw_data);
        ...
        if (TEXTURE_TYPE_JPG == it->getType()) {
            ...
            if (raw_data_length != fread(raw_data, 1, raw_data_length, fp)) {
                printf("[process_bc_file] Read data error from file: %s", file_name);
                return;
            }
            write_jpg_file(*it);
        }
            ...
            for (int y=0; y<it->getHeight(); y++) {
                ...
                switch (it->getType()) {
                    ...
                    case TEXTURE_TYPE_I8:
                        ...
                        break;
                    default:
                        printf("[process_bc_file] Unknown texture: %s", it->toString().c_str());
                        return;
                        break;
                }
                if (row_data) {
...
void process_tex2(const char *file_name, long file_size, FILE *fp)
{
    process_tex1(file_name, file_size, fp);
}

void process_bc_file(const char *file_name)
{
	unsigned char fcc[4];
    cout << "Processing " << file_name << " ..." << endl;
    FILE *fp = fopen(file_name, "rb");
    if (!fp) {
        printf("[process_bc_file] File %s cound not be opened for reading", file_name);
        return;
    }
    fseek(fp, 0, SEEK_END);
    ...
    if (0 == memcmp(&fcc, FCC_TEX1, sizeof(fcc))) {
        ...
        printf("[process_bc_file] File %s is not recognized as a BC file, unknown FCC: %c%c%c%c", file_name, fcc[0], fcc[1], fcc[2], fcc[3]);
        return;
    }
    cout << "\nDone" << endl;
}
