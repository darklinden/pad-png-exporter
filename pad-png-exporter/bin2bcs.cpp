...
void write_bc_file(const BlockList &blockList, const std::string &file_name) {
    ...
    if (!fp) {
        printf("[write_bc_file] File %s cound not be opened for writing", file_name.c_str());
        return;
    }
...
void process_bin_file(const char *file_name) {
    int seq = 0;
	FILE *fp = fopen(file_name, "rb");
	if (!fp) {
		printf("[read_bin_file] File %s cound not be opened for reading", file_name);
        return;
	}
    fseek(fp, 0, SEEK_END);
    long file_size = ftell(fp);
    if (file_size % BLOCK_SIZE) {
        printf("[read_bin_file] File %s bad size", file_name);
        return;
    }
    fseek(fp, 0, SEEK_SET);
    Block block;
    ...
    for (int i=0; i<file_size/BLOCK_SIZE; ++i) {
        if (BLOCK_SIZE != fread(&(block.front()), 1, BLOCK_SIZE, fp)) {
            printf("[read_bin_file] Fild %s read error", file_name);
            return;
        }
